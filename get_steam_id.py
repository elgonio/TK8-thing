# How to run: python3 get_steam_id.py <directory-containing-json> name_to_steamid.json
# Example:    python3 get_steam_id.py ./complete_JSONS_03_01 name_to_steamid.json
...
# Steam/PC platform code in your JSON
STEAM_PLATFORM_CODE = 3
...
def main(input_dir: str, output_filename: str):
    ...
    for file_ in files:
        ...
        name_to_steamid = {}
        for r in replays:
            if r.get("1pPlatform") == STEAM_PLATFORM_CODE:
                name, sid = r.get("1pPlayerName"), r.get("1pOnlineId")
                if name and sid is not None:
                    name_to_steamid[name] = str(sid)
            if r.get("2pPlatform") == STEAM_PLATFORM_CODE:
                name, sid = r.get("2pPlayerName"), r.get("2pOnlineId")
                if name and sid is not None:
                    name_to_steamid[name] = str(sid)

        print(f"✅ Extracted {len(name_to_steamid)} PC players from {file_.name}")
    ...
    out_path.write_text(json.dumps(name_to_steamid, indent=2), encoding="utf-8")
    print(f"📝 Wrote mapping to {out_path}")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python extract_id.py <input_dir> <name_to_steamid.json>", file=sys.stderr)
        sys.exit(1)
